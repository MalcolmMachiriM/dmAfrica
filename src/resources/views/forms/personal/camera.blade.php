...
<form action="{{ route('camera-insurance.store') }}" method="post" name="camera" id="camera" onsubmit="MM_validateForm('email','','RisEmail','people','','NisNum'); submitForm(event);"> 

  <h1>Quote For Camera Insurance</h1>
...
@if (session('success'))
        ...
        @endif
        <form action="{{ route('camera-insurance.store') }}" method="POST">
        @csrf
  <!--   1  -->
